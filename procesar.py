...
# Buscar Excel en la raíz y en data/
archivos = (glob.glob('*.xlsx') + glob.glob('*.xls') +
            glob.glob('data/*.xlsx') + glob.glob('data/*.xls'))

# Excluir archivos temporales de Excel (~$archivo.xlsx)
...
faltantes = [n for n,k in [('Fecha',kFecha),('Cliente',kCliente),
             ('Envase',kEnvase),('Movimiento',kMov),('Unidades',kUds)] if not k]
if faltantes:
...
for _, r in agg.iterrows():
    if not r['_fecha'] or r['_fecha'] == 'NaT':
        continue
    records.append({
        'f': r['_fecha'],
        'c': r['_cliente'],
...
print(f"data.json generado: {len(records)} registros ({len(output)/1024:.1f} KB)")
